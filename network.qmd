...
You may want to crop it by using [`tidytransit::filter_feed_by_area()`](https://r-transit.github.io/tidytransit/reference/filter_feed_by_area.html)
...
Anyway, you can create this file with some GIS operations, simplified by [`gtfsrouter::gtfs_transfer_table()`](https://urbananalyst.github.io/gtfsrouter/reference/gtfs_transfer_table.html)
...
[`GTFShift::unify()`](https://u-shift.github.io/GTFShift/reference/unify.html) proposes a simple solution to this problem, generating an aggregated GTFS file given several instances of these.

The unification produces a single GTFS instance, saved as a ZIP file.
Option `create_transfers` enables the generation of `transfers.txt`, aggregating close stops, even if from different GTFS.
...
The dem should be renamed to `.tif` (**single F**) instead of `.tiff`, otherwise [it will be ignored](https://github.com/ipeaGIT/r5r/issues/520) in r5r build.
:::
